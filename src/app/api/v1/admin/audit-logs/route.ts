...
import { listAdminAuditLogs } from '@/lib/services/admin-platform.service'
export const GET = withErrorHandler(async (req: Request) => {
  await requireAdminPermission(req, PERMISSIONS.AUDIT_READ)
  const input = parseAdminQuery(req, adminPlatformQuerySchema)
  const result = await listAdminAuditLogs(input)
  return ok({ logs: result.rows }, paginationMeta(input.page, input.limit, result.total))
})
