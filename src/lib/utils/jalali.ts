// ============================================
// Zar30 - Jalali (Shamsi) Date Helpers
// ============================================
// تبدیل تاریخ جلالی ↔ میلادی — الگوریتم استاندارد jalaali (بهداد اسفهبد)
// برای فرم‌های KYC که تاریخ تولد به جلالی وارد می‌شود
// ============================================
...
// نقاط شکست چرخه ۲۸۲۰ ساله جلالی
const BREAKS = [
...
// جلالی → Date میلادی — null اگر نامعتبر
export function jalaliToGregorian(jy: number, jm: number, jd: number): Date | null {
  if (!Number.isInteger(jy) || !Number.isInteger(jm) || !Number.isInteger(jd)) return null
  if (jy < 1200 || jy > 1500 || jm < 1 || jm > 12) return null
